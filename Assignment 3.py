...
class my_hash_set:
    ...
    def __contains__(self, key):
        g = hash(key) % self.__limit
        if self.__items[g]:
           cheese = self.__items[g]
           for j in cheese:
                if j[0] == key:
                    return True
           return False
    ...
    def __getitem__(self, key):
        jeeves = hash(key) % self.__limit
        if self.__items[jeeves]:
            for q in self.__items[jeeves]:
                if q[0] == key:
                    return q[1]
        return None
    def __delitem__(self, key):
       w = hash(key) % self.__limit
       if self.__items[w]:
            for u in self.__items[w]:
                if u[0] == key:
                    u[0] == None
                    u[1] == None
                    self.__count -= 1
